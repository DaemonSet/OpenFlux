...
func (c *MaxClient) LoginByToken(token string) error {
	if _, err := c.invoke(6, map[string]interface{}{
		...
		return fmt.Errorf("initialize MAX session: %w", err)
	}
	...
	if !ok {
		return fmt.Errorf("login response has no profile object (payload fields: %v)", mapKeys(payload))
	}
	contact, ok := profile["contact"].(map[string]interface{})
	if !ok {
		return fmt.Errorf("login response has no profile.contact object (profile fields: %v)", mapKeys(profile))
	}
	contactID, ok := contact["id"].(float64)
	if !ok || contactID <= 0 {
		return fmt.Errorf("login response has no valid profile.contact.id (contact fields: %v)", mapKeys(contact))
	}
	fmt.Printf("[MAX] Authenticated profile.contact.id: %d\n", int64(contactID))
	if err := c.initializeCallsAPI(); err != nil {
		return err
	}
	c.loggedIn = true
	go c.keepalive()
