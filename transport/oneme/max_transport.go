...
func (t *OneMeTransport) Start() error {
	...
	if err := t.oneMeClient.LoginByToken(t.token); err != nil {
		return fmt.Errorf("login to MAX: %w", err)
	}
	if !t.exit && strconv.FormatInt(t.uid, 10) == t.oneMeClient.callsLogin.ExternalUserID {
		return fmt.Errorf("--maxUid %d identifies the authenticated client itself; use the exit account's Calls external user ID", t.uid)
	}
