...
func startOutgoingCall(client *MaxClient, calleeID int64) *CallHandler {
	...
	go func() {
		for {
			...
			endpoint, err := client.startConversation(calleeID)
			if err != nil {
				logError("[CALLER] start conversation failed: %v", err)
				return
			}
			conn, _, err := websocket.DefaultDialer.Dial(endpoint, nil)
			if err != nil {
				logError("[CALLER] Dial error: %v", err)
				return
			}
			h.mu.Lock()
			h.conn = conn
			...
			// Do not create a fresh MAX call in a tight reconnect loop.
			<-h.reconnectCh
			logError("[CALLER] Signaling disconnected; stopping current call")
			return
		}
	}()
