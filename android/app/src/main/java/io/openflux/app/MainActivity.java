...
public final class MainActivity extends Activity {
    ...
    @Override protected void onCreate(Bundle state) {
        ...
        if (storedDns == null
                ...
                || "1.1.1.1".equals(storedDns)) {
            dnsServer = DEFAULT_DNS;
            prefs.edit().putString("dns_server", dnsServer).apply();
        } else {
            dnsServer = storedDns;
        }
    ...
    private View buildSettingsPage() {
        ...
        TextView encryptionHint = text(
                "Одинаковый ключ шифрования должен быть настроен на телефоне и VDS.",
                11, secondary, false);
    ...
    private View buildEncryptionField() {
        ...
        encryptionInput.setPadding(dp(16), 0, dp(16), 0);
    ...
    private void toggleVpn() {
        ...
        if (encryptionSecret == null || encryptionSecret.length() < 16) {
            Toast.makeText(this, "Ключ шифрования должен содержать не менее 16 символов", Toast.LENGTH_LONG).show();
            showPage(PAGE_SETTINGS);
            return;
        }
        persistSettings();
